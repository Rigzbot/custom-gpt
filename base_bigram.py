...
from config import block_size, batch_size, device, n_embd, eval_iters, learning_rate, max_iters, eval_interval
...
"""Character level encoder for now, but will change this later to use BPE"""
...
def get_batch(split):
    ...
    ix = torch.randint(len(data) - block_size, (batch_size,)) # this will be starting index of the random patch (there will be 4 numbers since batch size is 4)
...
context = torch.zeros((1, 1), dtype=torch.long, device=device) # starting contect of '/n'
